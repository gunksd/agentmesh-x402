...
const PROOF_POINTS = [
  { label: "Settlement", value: "BNB Smart Chain" },
  { label: "Protocol", value: "x402 v2 · Permit2" },
  { label: "Cost per run", value: formatUsd(PIPELINE_TOTAL_USD) },
  { label: "Gas paid by payer", value: "None" },
];
    ...
    <header className="relative overflow-hidden border-b border-[var(--border)]">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-6 pb-16 pt-16 sm:pb-24 sm:pt-24">
        ...
        <h1 className="mt-5 max-w-3xl text-[34px] font-semibold leading-[1.1] tracking-[-0.02em] sm:text-[52px]">
          ...
          <span className="block text-[var(--brand)]">and settle the bill on-chain</span>
        </h1>

        <p className="mt-5 max-w-2xl text-[14px] leading-relaxed text-[var(--muted)] sm:text-[15px]">
          AgentMesh is a network of specialist agents that sell their work over
          HTTP. An orchestrator discovers what it needs, receives a{" "}
          <span className="numeric text-[var(--foreground)]">402 Payment Required</span>,
          signs an EIP-712 authorisation, and the invoice settles on BNB Smart
          Chain in seconds. Every hop is a real transfer with a verifiable
          receipt, and no step waits on a human.
        </p>
